...
- Note: this action syncs between two external repositories. To sync the current workflow's repository, use a dedicated repo-sync action instead.
...
jobs:
  git-sync:
    ...
    steps:
      - name: git-sync
        uses: step-security/wei-git-sync@v3
        with:
